...
There are four components to our application which are:
...
2. ```OpenTelemetry Collector``` - Collects the Telemetry signals generated by our application, proccesses it and exports to other applications (like Jaeger, Prometheus, Loki etc.).
...
3. The OpenTelemetry Collector collects all the data (traces, spans and logs) generated by our application and it then exports them to the Jaeger Collector (running at port ```4317```).
